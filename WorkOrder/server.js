...
// Static files
// ────────────────────────────────────────────────────────────
// Serve Workorder UI at both "/" and "/Workorder" so the app stays reachable
// even when mounted under a path prefix (e.g., https://domain.com/Workorder/).
app.use(express.static(PUBLIC_DIR));
app.use('/Workorder', express.static(PUBLIC_DIR));

// Serve the existing Timesheets UI from its folder when using the Render-hosted
// server as the single entry point for both tools.
app.use('/Timesheets', express.static(TIMESHEETS_DIR));

// SPA-style fallbacks to load the correct index file when a deep link is used.
app.get(['/Workorder', '/Workorder/*'], (_req, res) => {
